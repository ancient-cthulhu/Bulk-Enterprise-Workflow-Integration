...
1. Install the `veracode-workflow-app` GitHub App
...
|`--install-app`                  |+ `admin:enterprise`                 |

Full rollout with all flags: `read:org`, `admin:org`, `read:enterprise`, `repo`, `workflow`, `admin:enterprise`

> `admin:enterprise` is only required for `--install-app`. If installing the app manually via `manual_install_links.csv`, `read:enterprise` is sufficient throughout.
...
1. **No flag** - falls back to `/user/orgs` (all orgs accessible to the token)
1. **`--orgs-file FILE`** - explicit list, used as filter when combined with `--enterprise`
